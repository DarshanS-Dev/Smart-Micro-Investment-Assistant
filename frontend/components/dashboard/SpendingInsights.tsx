...
export function SpendingInsights({ categories }: { categories: CategoryInsight[] }) {
  ...
  return (
    <section aria-label="Spending insights" className="py-8">
      <h2 className="mb-4 font-display text-xl font-semibold">Spending insights</h2>

      {sorted.length === 0 ? (
        // Same empty-state pattern as GrowthChart ("Not enough history yet")
        // and LedgerTable ("No transactions yet") — dashed border, muted
        // copy, no hidden section. A category only appears here once its
        // round-up has actually funded an executed investment (see
        // portfolio.py), so this is the honest "nothing invested from a
        // category yet" state, not a broken chart.
        <div className="flex h-64 items-center justify-center border border-dashed border-ink/20 text-sm text-ink/45">
          Not enough invested history yet — this fills in once a category&apos;s
          round-ups cross the threshold.
        </div>
      ) : (
        <>
